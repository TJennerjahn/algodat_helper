<script>
  ...
  const selection = (input) => {
    ...
    while (workingCopy.length > 0) {
      ...
      let equalActivities = workingCopy.filter((el) =>
        el.end.isSame(workingCopy[0].end)
      );
      ...
      workingCopy = workingCopy.filter((el) =>
        el.start.isSameOrAfter(result[result.length - 1].end)
      );
    }
