 ...
 */
class BTree {
    constructor(order, sidePreferenceForDeletion) {
        /** @type {number} */
        this.order = order;
        /** 
        ...
        */
        ...
        this.sidePreferenceForDeletion = sidePreferenceForDeletion;
    }
    ...
    deleteFromNode(node, value) {
        ...
        if (index >= 0) {
            ...
            if (node.children[index].n > this.order - 1 ||
                ...
                if (node.children[index].n > this.order - 1) {
                    if (this.sidePreferenceForDeletion === 0) {
                    ...
                    } else if (this.sidePreferenceForDeletion === 1) {
                        // Replace the target value for the higher of left node.
                        // Then delete that value from the child
                        const predecessor = this.getMinMaxFromSubTree(node.children[index + 1], 0);
                        node.values[index] = predecessor;
                        return this.deleteFromNode(node.children[index + 1], predecessor);
                    }
   ...
   */
    ...
    asGraph(node) {
        ...
        const printGivenLevel = (node, level) => {
            ...
            if (level === 1) {
                inOrderData.push(...node.values);
                graph.push({
                    group: "nodes",
                    data: {
                        id: node.id,
                        label: node.values.toString().replaceAll(",", "|"),
                    }
                })
            } else if (level > 1) {
